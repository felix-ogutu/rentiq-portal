...
import { Search, ChevronLeft, ChevronRight, Wrench, Calendar, FileText } from 'lucide-react';
import { Button } from '../ui/button';
import { Input } from '../ui/input';
import { Card, CardContent } from '../ui/card';

interface ServiceProviderMaintenanceViewProps {
  maintenanceRequests: MaintenanceRequest[];
  properties: Property[];
}

export function ServiceProviderMaintenanceView({ maintenanceRequests, properties }: ServiceProviderMaintenanceViewProps) {
  ...
  const assignedRequests = maintenanceRequests.filter(r => r.assignedTo === 'Service Provider');

  // Filter maintenance requests
  const filteredRequests = useMemo(() => {
    return assignedRequests.filter(request => {
      const matchesSearch = request.issue.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  }, [assignedRequests, searchQuery, statusFilter, priorityFilter]);
  ...
  const handleViewDetails = (requestId: string) => {
    ...
    alert('View maintenance details - Feature coming soon!');
  };

  const handleUpdateStatus = (requestId: string) => {
    console.log('Update status:', requestId);
    alert('Update maintenance status - Feature coming soon!');
  };

  // Stats by category
  const plumbingCount = assignedRequests.filter(r => r.category === 'plumbing').length;
  const electricalCount = assignedRequests.filter(r => r.category === 'electrical').length;
  const hvacCount = assignedRequests.filter(r => r.category === 'hvac').length;
  const otherCount = assignedRequests.filter(r => !['plumbing', 'electrical', 'hvac'].includes(r.category)).length;
    ...
    <div className="space-y-6 p-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-gray-600">View and manage all maintenance tasks</p>
        </div>
      </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-3xl font-bold text-blue-600">{plumbingCount}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-3xl font-bold text-yellow-600">{electricalCount}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-3xl font-bold text-green-600">{hvacCount}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-3xl font-bold text-purple-600">{otherCount}</p>
              </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="text-left py-4 px-6 font-semibold text-sm text-gray-700">Request ID</th>
                ...
                <th className="text-left py-4 px-6 font-semibold text-sm text-gray-700">Reported By</th>
              ...
              {paginatedRequests.map((request, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-4 px-6">
                      <p className="text-gray-900 max-w-xs truncate">{request.issue}</p>
                    </td>
                    <td className="py-4 px-6">
                      <p className="text-gray-700">{getPropertyName(request.propertyId)}</p>
                    </td>
                    <td className="py-4 px-6">
                      <p className="text-gray-700">{request.unitNumber}</p>
                    </td>
                    <td className="py-4 px-6">
                      <p className="text-gray-700">{request.reportedBy}</p>
                    </td>
                    <td className="py-4 px-6">
                      <p className="text-gray-700 capitalize">{request.category}</p>
                    </td>
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${getPriorityBadgeClass(request.priority)}`}>
                        {request.priority}
                      </span>
                    </td>
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${getStatusBadgeClass(request.status)}`}>
                        {request.status}
                      </span>
                    </td>
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-1 text-gray-700">
                        <Calendar size={14} />
                        <p className="text-sm">{request.reportedDate}</p>
                      </div>
                    </td>
                    <td className="py-4 px-6">
